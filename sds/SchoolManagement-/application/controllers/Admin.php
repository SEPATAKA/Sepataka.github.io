...
class Admin extends CI_Controller
{
	...
	function online_admission($param1 = '', $param2 = '', $param3 = '')
     {
		...
		if($_FILES['userfile']['name'] != '') {
	        ...
	        $data1 = array(
	           ...
	           'quiztype' => $this->input->post('quiztype'.$i, TRUE),
	...
	/*	if($_FILES['userfile']['userfile'] != '') {
		...
		$copied = copy($_FILES['userfile']['tmp_name'], $newname);
           ...
           // move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/student_image/'.$photo);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            //$this->email_model->account_opening_email('tutor', $data['email']); //SEND EMAIL ACCOUNT OPENING EMAIL
        ...
        if ($param1 == 'do_update') {
            ...
            $data['phone']       = $this->input->post('phone');
            $data['email']       = $this->input->post('email');
            $this->db->where('tutor_id', $param2);
            $this->db->update('tutor', $data);
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/tutor_image/' . $param2 . '.jpg');
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/online_admission/', 'refresh');
        } else if ($param1 == 'personal_profile') {
            $page_data['personal_profile']   = true;
            $page_data['current_tutor_id'] = $param2;
        } else if ($param1 == 'edit') {
            $page_data['edit_data'] = $this->db->get_where('tutor', array(
                'tutor_id' => $param2
            ))->result_array();
        }
        if ($param1 == 'delete') {
	...
	function student_bulk_add($param1 = '')
	{
		...
		if ($param1 == 'import_excel')
		{
			...
			foreach( $xlsx->rows() as $r )
			{
				...
				{
					if ($i == 0)	    $data['student_no']			=	$r[$i];
					else if ($i == 1)	$data['surname']		=	$r[$i];
					else if ($i == 2)	$data['initals']		    =	$r[$i];
					else if ($i == 3)	$data['subject_id']		=	$r[$i];
					else if ($i == 4)	$data['phone']			=	$r[$i];
					else if ($i == 5)	$data['email']			=	$r[$i];
					else if ($i == 6)	$data['Reason']		=	$r[$i];
					else if ($i == 7)	$data['Information_Session']		    =	$r[$i];
					else if ($i == 8)	$data['Attendance_id']		=	$r[$i];
					else if ($i == 10)$data['Supported_Subjects']			=	$r[$i];
					else if ($i == 11)	$data['WhatsApp_Status']			=	$r[$i];
					else if ($i == 12)	$data['tutor_name']		=	$r[$i];
					else if ($i == 13)	$data['roll']			=	$r[$i];
          else if ($i == 13)	$data['subject_name']	=	$r[$i];
		...
		$page_data['page_title'] = 'Add Spreedsheet Student';
		$this->load->view('backend/index', $page_data);
	}
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param2 == 'do_update') {
            ...
            $data['Information_Session']         = $this->input->post('Information_Session');
            ...
            $data['parent_id']   = $this->input->post('parent_id');
            $data['roll']        = $this->input->post('roll');
            $data['Information_Session_Attendance'] = $this->input->post('Information_Session_Attendance');
            $data['Supported_Subjects']= $this->input->post('Supported_Subjects');
    ...
    /****MANAGE tutorS*****/
    function tutor($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            ...
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/tutor_image/' . $tutor_id . '.jpg');
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            $this->email_model->account_opening_email('tutor', $data['email']); //SEND EMAIL ACCOUNT OPENING EMAIL
            redirect(base_url() . 'index.php?admin/tutor/', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']        = $this->input->post('name');
            $data['birthday']    = $this->input->post('birthday');
        ...
        $page_data['tutors']   = $this->db->get('tutor')->result_array();
        $page_data['page_name']  = 'tutor';
        $page_data['page_title'] = 'Manage tutor';
        $this->load->view('backend/index', $page_data);
    }
    ...
    /****MANAGE QUIZS*****/
    function quiz($param1 = '', $param2 = '' , $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/quiz/', 'refresh');
        }
        if ($param1 == 'edit' && $param2 == 'do_update') {
            ...
            $this->db->where('quiz_id', $param3);
            $this->db->update('quiz', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/quiz/', 'refresh');
        } else if ($param1 == 'edit') {
            $page_data['edit_data'] = $this->db->get_where('quiz', array(
                'quiz_id' => $param2
            ))->result_array();
        }
        if ($param1 == 'delete') {
        ...
        $page_data['quizs']      = $this->db->get('quiz')->result_array();
        $page_data['page_name']  = 'quiz';
        $page_data['page_title'] = 'Manage QUIZ';
        $this->load->view('backend/index', $page_data);
    }

    /****** SEND QUIZ MARKS VIA SMS ********/
    function quiz_marks_sms($param1 = '' , $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($param1 == 'send_sms') {
            ...
            // get the marks of the student for selected quiz
            ...
            redirect(base_url() . 'index.php?admin/quiz_marks_sms' , 'refresh');
        }

        $page_data['page_name']  = 'quiz_marks_sms';
        $page_data['page_title'] = 'Send Marks by SMS';
        $this->load->view('backend/index', $page_data);
    }

    /****MANAGE QUIZ MARKS*****/
    function marks($quiz_id = '', $class_id = '', $subject_id = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');

        if ($this->input->post('operation') == 'selection') {
            $page_data['quiz_id']    = $this->input->post('quiz_id');
            $page_data['class_id']   = $this->input->post('class_id');
            $page_data['subject_id'] = $this->input->post('subject_id');

            if ($page_data['quiz_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?admin/marks/' . $page_data['quiz_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose quiz, class and subject');
        ...
        if ($this->input->post('operation') == 'update') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/marks/' . $this->input->post('quiz_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id'), 'refresh');
        }
        $page_data['quiz_id']    = $quiz_id;
        $page_data['class_id']   = $class_id;
        $page_data['subject_id'] = $subject_id;
        $page_data['page_info'] = 'QUIZ marks';
        $page_data['page_name']  = 'marks';
        $page_data['page_title'] = 'Manage QUIZ Marks';
        $this->load->view('backend/index', $page_data);
    }
	...
	function manage_attendance($date='',$month='',$year='',$class_id='')
	{
		...
		if($_POST)
		{
			...
			$this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
			redirect(base_url() . 'index.php?admin/manage_attendance/'.$date.'/'.$month.'/'.$year.'/'.$class_id.'/'.$student_id, 'refresh');
		}
        $page_data['date']     =	$date;
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if ($check_sms_send == 1) {
                ...
                $tutors = $this->db->get('tutor')->result_array();
